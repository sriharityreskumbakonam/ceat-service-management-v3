...
const state={records:[],page:"dashboard",savedRecord:null};
...
const recordNo=()=>String(state.records.length+1).padStart(3,"0");
...
$("#logoutBtn").onclick=()=>toast("This V3 version has no separate login screen.");
...
function serviceFormHTML(){
 ...
 <div class="grid2">
  <div><label>Service Record No.</label><input id="recordNo" value="${recordNo()}" readonly></div>
  <div><label>Date</label><input id="recordDate" type="date" value="${today()}"></div>
  <div><label>Customer Name <span class="opt">(Optional)</span></label><input id="customerName" placeholder="Enter customer name"></div>
  <div><label>Mobile Number <span class="opt">(Optional)</span></label><input id="mobile" inputmode="numeric" placeholder="10-digit mobile number"></div>
  <div><label>Vehicle Model <span class="req">*</span></label><input id="vehicleModel" placeholder="Eg. Tata Nexon"></div>
  <div><label>Vehicle Number <span class="req">*</span></label><input id="vehicleNumber" placeholder="Eg. TN49AB1234"></div>
  <div><label>Current KM Reading <span class="req">*</span></label><input id="currentKm" type="number" min="0" placeholder="Enter current KM"></div>
  <div><label>Service Classification <span class="req">*</span></label><select id="classification"><option value="">Select classification</option><option>Free Service</option><option>Regular Service</option><option>Not Applicable</option></select></div>
 </div>
 ...
 <div class="service-table"><div class="service-head"><div>SERVICE / ITEM</div><div>QTY</div><div>RATE (₹)</div><div>AMOUNT (₹)</div></div><div id="serviceRows">${serviceRowsHTML()}</div></div>
 <div class="totals"><div class="total-line"><span>Subtotal</span><b id="subtotal">₹0.00</b></div>
 <div class="total-line"><span>Discount</span><input id="discount" type="number" min="0" step="0.01" value="0"></div>
 <div class="total-line grand"><span>Grand Total</span><b id="grandTotal">₹0.00</b></div></div>
 </section>
 ...
 <div class="bottom-actions"><button class="btn" id="clearBtn">Clear</button><button class="btn blue" id="printBtn" disabled>Print Service Record</button><button class="btn green" id="shareBtn" disabled>Share PDF via WhatsApp</button><button class="btn primary" id="saveBtn">Save Service Record</button></div>`;
}
function getForm(){
...
function bindService(){
 document.addEventListener("input",calc,{once:false});
 $("#clearBtn").onclick=()=>{renderService();};
...
async function saveRecord(){
 ...
 try{
   const r=push(recordsRef);const data={...f,savedAt:new Date().toISOString(),firebaseKey:r.key};
   await set(r,data);state.savedRecord={id:r.key,...data};
   $("#printBtn").disabled=false;$("#shareBtn").disabled=!navigator.share;
   showSaved(f.recordNo);
 }catch(e){console.error(e);alert("Could not save the service record. Please check Firebase Database Rules and connection.");}
 finally{btn.disabled=false;btn.textContent="Save Service Record"}
}
...
function openPrint(f){const w=window.open("","_blank","width=900,height=1100");if(!w){alert("Please allow pop-ups for printing.");return}w.document.write(printHTML(f));w.document.close()}
...
function downloadExcelBackup(sourceRecords=state.records){
 ...
 const rows=sourceRecords.map(r=>({"Service Record No.":r.recordNo||"","Date":r.date||"","Customer Name":r.customerName||"","Mobile Number":r.mobile||"","Vehicle Model":r.vehicleModel||"","Vehicle Number":r.vehicleNumber||"","Current KM":Number(r.currentKm||0),"Service Classification":r.classification||"","Wheel Alignment":(r.services||[]).find(x=>x.name==="Wheel Alignment")?.amount||0,"Wheel Balancing":(r.services||[]).find(x=>x.name==="Wheel Balancing")?.amount||0,"Rotation":(r.services||[]).find(x=>x.name==="Rotation")?.amount||0,"Subtotal":Number(r.subtotal||0),"Discount":Number(r.discount||0),"Grand Total":Number(r.total||0),"Saved Date & Time":r.savedAt||""}));
...
function renderDashboard(){
 const total=state.records.length,todayCount=state.records.filter(r=>r.date===today()).length,free=state.records.filter(r=>r.classification==="Free Service").length,regular=state.records.filter(r=>r.classification==="Regular Service").length;
 $("#main").innerHTML=`<div class="hero"><h1>Dashboard</h1><div>Live Service Management for Sri Hari Tyres</div></div>
 <div class="stat-grid"><div class="stat"><div class="label">VEHICLES / RECORDS</div><div class="num">${total}</div></div><div class="stat"><div class="label">SERVICE RECORDS</div><div class="num">${total}</div></div><div class="stat"><div class="label">TODAY'S RECORDS</div><div class="num">${todayCount}</div></div><div class="stat"><div class="label">REMINDER BASELINES</div><div class="num">${free+regular}</div></div></div>
...
function renderVehicles(){
 ...
 <section class="card"><div class="searchbar"><input class="search" id="vehicleSearch" placeholder="Search vehicle number, customer, mobile or model"></div>
 <div id="vehicleList">${vehicleRows(Object.values(latest))}</div></section>`;
 $("#newVehicleRecord").onclick=()=>setPage("service");
 $("#vehicleSearch").oninput=e=>{
   const q=e.target.value.toLowerCase().trim();
   $("#vehicleList").innerHTML=vehicleRows(Object.values(latest).filter(r=>[r.vehicleNumber,r.customerName,r.mobile,r.vehicleModel].join(" ").toLowerCase().includes(q)));
 };
}
...
function historyRows(arr){
 ...
 <td class="num">${money(r.total)}</td><td><button class="btn" data-print="${r.id}">Print</button></td></tr>`).join("")}</tbody></table></div>`;
}

function addMonths(dateStr,m){const d=new Date(dateStr+"T00:00:00");d.setMonth(d.getMonth()+m);return d.toISOString().slice(0,10)}
function reminderFor(r){
 if(r.classification==="Free Service")return {type:"Free Service",dueKm:Number(r.currentKm)+2000,dueDate:addMonths(r.date,1),rule:"KM + 2,000 OR 1 month, whichever comes first."};
 if(r.classification==="Regular Service")return {type:"Regular Service",dueKm:Number(r.currentKm)+5000,dueDate:null,rule:"KM + 5,000 only."};
 return null;
}
function renderReminders(){
 const latest={};state.records.forEach(r=>{if(!latest[r.vehicleNumber]||new Date(r.savedAt)>new Date(latest[r.vehicleNumber].savedAt))latest[r.vehicleNumber]=r});
 const arr=Object.values(latest).map(r=>({r,rem:reminderFor(r)})).filter(x=>x.rem);
 $("#main").innerHTML=`<div class="page-head"><h1>Service Reminders</h1></div><section class="card"><div class="notice">If this service has already been completed, please kindly disregard this reminder. We appreciate your continued trust in Sri Hari Tyres.</div>${arr.length?arr.map(x=>reminderCard(x.r,x.rem)).join(""):'<div class="empty">No reminder baselines available yet.</div>'}</section>`;
}
function reminderCard(r,rem){
 const dueDate=rem.dueDate&&today()>=rem.dueDate, text=`Dear ${r.customerName||"Customer"},\n\nThis is a friendly service reminder from Sri Hari Tyres for your vehicle ${r.vehicleNumber}.\n\nRecommended ${rem.type}: around ${Number(rem.dueKm).toLocaleString("en-IN")} KM${rem.dueDate?` or by ${prettyDate(rem.dueDate)}, whichever comes first.`:"."}\n\nIf this service has already been completed, please kindly disregard this reminder.\n\nThank you for trusting Sri Hari Tyres.`;
 return `<div class="reminder"><div><h3>${esc(r.vehicleNumber)} — ${esc(r.customerName||"Customer")}</h3><small>${esc(rem.type)} • Baseline: ${Number(r.currentKm).toLocaleString("en-IN")} KM on ${prettyDate(r.date)} • Next: ${Number(rem.dueKm).toLocaleString("en-IN")} KM${rem.dueDate?` / ${prettyDate(rem.dueDate)}`:""}</small></div><div><span class="badge ${dueDate?"due":"ok"}">${dueDate?"TIME DUE":"TRACKING"}</span> ${r.mobile?`<button class="btn green" onclick='sendReminder(${JSON.stringify(normalizePhone(r.mobile))},${JSON.stringify(text)})'>WhatsApp</button>`:""}</div></div>`;
}
