...
function serviceFormHTML(){
 ...
 const value=(key,def="")=>esc(e?.[key]??def);
 const selected=(v)=>e?.classification===v?" selected":"";
 ...
 <div class="grid2">
  <div><label>Service Record No.</label><input id="recordNo" value="${value("recordNo",recordNo())}" readonly></div>
  <div><label>Date</label><input id="recordDate" type="date" value="${value("date",today())}"></div>
  <div><label>Customer Name <span class="opt">(Optional)</span></label><input id="customerName" value="${value("customerName")}" placeholder="Enter customer name"></div>
  <div><label>Mobile Number <span class="opt">(Optional)</span></label><input id="mobile" inputmode="numeric" value="${value("mobile")}" placeholder="10-digit mobile number"></div>
  <div><label>Vehicle Model <span class="req">*</span></label><input id="vehicleModel" value="${value("vehicleModel")}" placeholder="Eg. Tata Nexon"></div>
  <div><label>Vehicle Number <span class="req">*</span></label><input id="vehicleNumber" value="${value("vehicleNumber")}" placeholder="Eg. TN49AB1234"></div>
  ...
  <div><label>Service Classification <span class="req">*</span></label><select id="classification"><option value="">Select classification</option><option${selected("Free Service")}>Free Service</option><option${selected("Regular Service")}>Regular Service</option><option${selected("Not Applicable")}>Not Applicable</option></select></div>
 </div>
 ...
 </div><div class="notice" style="margin-top:12px">A service record updates the relevant reminder baseline only when it is saved with the matching service classification.</div>
 </section>
...
function showSaved(no,updated=false){
 const box=$("#saveStatus");box.innerHTML=`<div class="save-box"><div class="save-icon">✅</div><h3>Service Record ${updated?"Updated":"Saved"}</h3><p>Service Record No: <b>#${esc(no)}</b><br>${updated?"Updated successfully":"Saved successfully"} in the live database.</p><button id="savedOk">OK</button></div>`;box.style.display="flex";$("#savedOk").onclick=()=>box.style.display="none";
}
...
function openPrint(f){
  // Print inside the current page so mobile browsers do not block a popup.
  ...
  const doc=new DOMParser().parseFromString(printHTML(f),"text/html");
...
function renderReminders(){
 ...
 state.records.forEach(r=>{
   const key=r.vehicleNumber||r.id;
   if(!vehicles[key])vehicles[key]={latest:null,free:null,regular:null};
   const v=vehicles[key];
   if(!v.latest||new Date(r.savedAt||0)>new Date(v.latest.savedAt||0))v.latest=r;
   if(r.classification==="Free Service"&&(!v.free||new Date(r.savedAt||0)>new Date(v.free.savedAt||0)))v.free=r;
   if(r.classification==="Regular Service"&&(!v.regular||new Date(r.savedAt||0)>new Date(v.regular.savedAt||0)))v.regular=r;
 });
 const arr=[];
 Object.values(vehicles).forEach(v=>{
   if(v.free) arr.push({r:v.free,current:v.latest,rem:reminderFor(v.free,v.latest)});
   if(v.regular) arr.push({r:v.regular,current:v.latest,rem:reminderFor(v.regular,v.latest)});
 });
 $("#main").innerHTML=`<div class="page-head"><h1>Service Reminders</h1></div><section class="card"><div class="notice">If this service has already been completed, please kindly disregard this reminder. We appreciate your continued trust in Sri Hari Tyres.</div>${arr.length?arr.map(x=>reminderCard(x.r,x.rem,x.current)).join(""):'<div class="empty">No reminder baselines available yet.</div>'}</section>`;
}
function reminderFor(baseline,current){
 if(baseline.classification==="Free Service")return {type:"Free Service",dueKm:Number(baseline.currentKm)+2000,dueDate:addMonths(baseline.date,1),rule:"KM + 2,000 OR 1 month, whichever comes first.",currentKm:Number(current?.currentKm||baseline.currentKm)};
 if(baseline.classification==="Regular Service")return {type:"Regular Service",dueKm:Number(baseline.currentKm)+5000,dueDate:null,rule:"KM + 5,000 only.",currentKm:Number(current?.currentKm||baseline.currentKm)};
 return null;
}
function reminderCard(r,rem,current){
 const kmDue=rem.currentKm>=rem.dueKm;
 const dateDue=!!(rem.dueDate&&today()>=rem.dueDate);
 const due=kmDue||dateDue;
 const text=`Dear ${r.customerName||"Customer"},\n\nThis is a friendly service reminder from Sri Hari Tyres for your vehicle ${r.vehicleNumber}.\n\nRecommended ${rem.type}: around ${Number(rem.dueKm).toLocaleString("en-IN")} KM${rem.dueDate?` or by ${prettyDate(rem.dueDate)}, whichever comes first.`:"."}\n\nIf this service has already been completed, please kindly disregard this reminder.\n\nThank you for trusting Sri Hari Tyres.`;
 return `<div class="reminder"><div><h3>${esc(r.vehicleNumber)} — ${esc(r.customerName||"Customer")}</h3><small>${esc(rem.type)} • Baseline: ${Number(r.currentKm).toLocaleString("en-IN")} KM on ${prettyDate(r.date)} • Current: ${Number(rem.currentKm).toLocaleString("en-IN")} KM • Next: ${Number(rem.dueKm).toLocaleString("en-IN")} KM${rem.dueDate?` / ${prettyDate(rem.dueDate)}`:""}</small></div><div><span class="badge ${due?"due":"ok"}">${due?"SERVICE DUE":"TRACKING"}</span> ${r.mobile?`<button class="btn green" onclick='sendReminder(${JSON.stringify(normalizePhone(r.mobile))},${JSON.stringify(text)})'>WhatsApp</button>`:""}</div></div>`;
}
