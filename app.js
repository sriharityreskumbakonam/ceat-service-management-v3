...
import { getDatabase, ref, push, set, onValue, remove, runTransaction } from "https://www.gstatic.com/firebasejs/12.1.0/firebase-database.js";
...
async function initFirebase(){
  try{
    ...
    onValue(recordsRef,s=>{
      const v=s.val()||{};
      state.records=Object.entries(v).map(([id,r])=>({id,...r}))
        .sort((a,b)=>(b.date||"").localeCompare(a.date||"")||(b.savedAt||"").localeCompare(a.savedAt||""));
      renderIfData();
    },err=>{
      console.error("serviceRecords read failed:",err);
      toast("Cannot read service records: "+(err?.message||"Permission denied"));
    });

    onValue(remindersRef,s=>{
      const v=s.val()||{};
      state.reminders=Object.entries(v).map(([id,r])=>({id,...r}))
        .filter(r=>r.status!=="sent"&&r.status!=="completed"&&r.status!=="cancelled")
        .sort((a,b)=>(a.dueDate||"").localeCompare(b.dueDate||""));
      renderIfData();
    },err=>{
      console.error("reminders read failed:",err);
    });
    ...
    fbReady=false;
    const dbStatus=document.getElementById("dbStatus");
    if(dbStatus){dbStatus.textContent="● DATABASE ERROR";dbStatus.style.background="#4b1717";dbStatus.style.color="#ffb0b0";}
    console.error("Firebase initialization failed:",err);
    toast("Database connection unavailable.");
  }
}
