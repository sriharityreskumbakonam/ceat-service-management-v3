...
async function saveRecord(){
 ...
 if(!notApplicable && (!f.vehicleModel||!f.vehicleNumber||!f.currentKm)){
   alert("Please enter Vehicle Model, Vehicle Number and Current KM."); return;
 }
 ...
 const fingerprint=saveFingerprint(f);
 if(!state.editingRecord && fingerprint===lastSavedFingerprint){
   toast("This service record is already saved."); return;
 }
 ...
 try{
   ...
   if(state.editingRecord){
     ...
     // Push creates the Firebase key locally; only ONE database write follows.
   ...
   // Do not use a Promise.race timeout that can leave the UI in an ambiguous
   // state. Firebase resolves this promise when the write is acknowledged.
   await set(target,data);

   state.savedRecord={id,...data};
   state.editingRecord=null;
   lastSavedFingerprint=saveFingerprint(f);

   // Immediately switch the UI to the saved state.
   showSaved(f.recordNo,isUpdate);

   // The WhatsApp/PDF actions are intentionally NOT triggered here.
   // They are separate buttons after saving.
   const printBtn=$("#printBtn");
   const whatsappBtn=$("#whatsappBtn");
   const sendRecordBtn=$("#sendRecordBtn");
   if(printBtn)printBtn.disabled=false;
   if(whatsappBtn)whatsappBtn.disabled=false;
   if(sendRecordBtn)sendRecordBtn.disabled=false;
   window._recordSavedThisForm=true;

   toast("Service record saved successfully.");
 }catch(e){
   console.error("Service record save failed:",e);
   alert("Could not save the service record. Firebase error: "+(e?.message||"Unknown error"));
 }finally{
