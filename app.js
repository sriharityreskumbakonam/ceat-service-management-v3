...
let db=null, recordsRef=null, remindersRef=null;
let fbReady=false;
...
const recordNo=()=>{const n=state.records.map(r=>parseInt(r.recordNo,10)).filter(Number.isFinite);return String((n.length?Math.max(...n):0)+1).padStart(3,"0")};
...
<div><label>Vehicle Model <span class="req">*</span></label><input id="vehicleModel" list="vehicleModels" value="${val("vehicleModel")}" placeholder="Eg. Tata Nexon"><datalist id="vehicleModels">${[...new Set(state.records.map(r=>r.vehicleModel).filter(Boolean))].sort().map(x=>`<option value="${esc(x)}">`).join("")}</datalist></div>
<div><label>Vehicle Number <span class="req">*</span></label><input id="vehicleNumber" list="vehicleNumbers" value="${val("vehicleNumber")}" placeholder="Eg. TN49AB1234"><datalist id="vehicleNumbers">${[...new Set(state.records.map(r=>r.vehicleNumber).filter(Boolean))].sort().map(x=>`<option value="${esc(x)}">`).join("")}</datalist></div>
...
<div><label>Technician <span class="req">*</span></label><select id="technician"><option value="">Select technician</option>${TECHNICIANS.map(t=>`<option value="${t}"${val("technician")===t?" selected":""}>${t}</option>`).join("")}</select></div>
</div><p class="hint">Select an existing vehicle number to reuse customer details. Vehicle number, model and KM are required.</p></section>
...
<div class="bottom-actions"><button class="btn" id="clearBtn">${e?"Cancel Edit":"Clear"}</button><button class="btn blue" id="printBtn" ${e?"":"disabled"}>🖨 Print Service Record</button><button class="btn green" id="whatsappBtn" disabled>💬 WhatsApp Customer</button><button class="btn primary" id="saveBtn">${e?"Update Service Record":"Save Service Record"}</button></div>`}
...
async function saveRecord(){
  ...
  if(!f.technician){alert("Please select Technician.");return}
  ...
  try{
    let id,data;
    const {ref,set,push}=window.firebaseFns;
...
function vehicleRows(arr){if(!arr.length)return'<div class="empty">No vehicles found.</div>';return arr.map(r=>{const h=state.records.filter(x=>x.vehicleNumber===r.vehicleNumber).sort((a,b)=>(b.date||"").localeCompare(a.date||"")),id="vh_"+r.id.replace(/[^A-Za-z0-9]/g,"_");return `<div class="vehicle-card"><div class="vehicle-head"><div><h3>${esc(r.vehicleNumber)}</h3><small>${esc(r.vehicleModel)} • ${h.length} record(s)</small></div><button class="btn" data-toggle-history="${id}">View History</button></div><div class="vehicle-details"><div class="detail-chip"><span>Customer</span><b>${esc(r.customerName||"—")}</b></div><div class="detail-chip"><span>Mobile</span><b>${esc(r.mobile||"—")}</b></div><div class="detail-chip"><span>Last Service</span><b>${prettyDate(h[0]?.date)}</b></div><div class="detail-chip"><span>Current KM</span><b>${Number(h[0]?.currentKm||r.currentKm||0).toLocaleString("en-IN")} KM</b></div></div><div id="${id}" class="history-expand" hidden>${historyRows(h,false)}</div></div>`}).join("")}
...
function reminderCard(x){
  ...
  const action=x.mobile?'<button class="btn green" data-reminder-send="'+esc(x.id)+'">WhatsApp</button>':'<span class="badge free">No Mobile</span>';
  return '<div class="reminder"><div><h3>'+esc(x.vehicleNumber)+' — '+esc(x.customerName||"Customer")+'</h3><small>'+esc(x.type)+' • Reminder: '+prettyDate(x.dueDate)+' • Reference KM: '+Number(x.dueKm||0).toLocaleString("en-IN")+' KM</small></div><div><span class="badge '+(due?'due':'ok')+'">'+(due?'DUE':'UPCOMING')+'</span> '+action+'</div></div>';
}
